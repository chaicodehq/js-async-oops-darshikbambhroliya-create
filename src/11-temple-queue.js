...
export class TempleQueue {
  ...
  static merge(queue1, queue2) {
    ...
    const maxCapacity = queue1.#devotees.length + queue2.#devotees.length + 10;
    const mergeQueue = new TempleQueue(newName, maxCapacity);
    for (const que of queue1.#devotees) {
      mergeQueue.#devotees.push({ ...que });
    }
    for (const que of queue2.#devotees) {
      mergeQueue.#devotees.push({ ...que });
    }
    return mergeQueue;
  }
