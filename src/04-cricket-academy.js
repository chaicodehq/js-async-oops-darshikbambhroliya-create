...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const TotalStrikeRate = this.innings.reduce((sum, rate) => {
      return (sum = sum + rate.strikeRate);
    }, 0);
    return TotalStrikeRate / this.innings.length;
  }

  getProfile() {
    const totalRuns = this.innings.reduce((sum, run) => {
      return (sum = sum + run.runs);
    }, 0);
    return {
      ...super.getProfile(),
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEco = this.spells.reduce((sum, spell) => {
      return (sum = sum + spell.economy);
    }, 0);
    const avgEco = totalEco / this.spells.length;
    return avgEco;
  }

  getProfile() {
    const totalWickets = this.spells.reduce((sum, wic) => {
      return (sum = sum + wic.wickets);
    }, 0);
    return {
      ...super.getProfile(),
